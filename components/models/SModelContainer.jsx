...
const SModelContainer = ({ canvasIsActive }) => {
  ...
  const handleStopDuring = useCallback(() => {
    lenis.stop();
    setTimeout(() => {
      lenis.start();
    }, 200);
  ...
  useGSAP(
    () => {
      ...
      new ScrollTrigger({
        ...sectionTriggerSettings,
        start: () =>
          `${(((6 / 865) * (window.innerHeight - 944) + 9) / 2840) * (window.innerWidth - 1920) + 48}% bottom`,
      });

      new ScrollTrigger({
        ...sectionTriggerSettings,
        start: () =>
          `${(((9 / 371) * (window.innerHeight - 944) + 31) / 14200) * (window.innerWidth - 1920) + 82.5}% bottom`,
      });
    ...
    { dependencies: [handleStopDuring], revertOnUpdate: true }
  );
